...
if __name__ == "__main__":
    ...
    def val_mixed_loss_fn():
        loss, _, _, _, _ = compute_drm_loss_CH(
                ...
                lambda_bc=LAMBDA_BC, lambda_ic=LAMBDA_IC_BEGIN,
            )
        return loss.item()
    ...
    for epoch in range(NUM_EPOCHS):
        LAMBDA_IC = LAMBDA_IC_BEGIN + (LAMBDA_IC_END - LAMBDA_IC_BEGIN) * epoch / NUM_EPOCHS
    ...
    optimizer = SALAdam(net_u.parameters(), lr=LEARNING_RATE, weight_decay=1e-4)
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=5000, gamma=0.999)
    # 2. 生成数据
    data = generate_data(N_f=800, N_b=800, N_i=1000, device=DEVICE,mode='forward')
