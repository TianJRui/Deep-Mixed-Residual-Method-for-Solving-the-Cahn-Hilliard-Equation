...
if __name__ == "__main__":
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if epoch % 100 == 0:
            ...
            print(f"mixed [Epoch {epoch}] Train Loss: {train_loss.item():.3e} "
                  ...
                  f"Val Loss: {val_loss:.3e},u_max = {torch.max(net_u_mixed(x_f_val, y_f_val, t_f_val)).item():.2e},u_max_i = {torch.max(net_u_mixed(x_i_val, y_i_val, t_i_val)).item():.2e}")
            if epoch % 1000 == 0:
    ...
    def plot_mlp_param_distribution(model: nn.Module, model_name: str = "MLP Model") -> None:
        """
        Plots parameter distribution with Symmetrical Logarithmic X-axis.
        Removed KDE calculation as requested.
        """
        
        # 1. Collect and aggregate all parameters
        all_params_list = []
        
        for name, param in model.named_parameters():
            if param.requires_grad:
                params_np = param.cpu().detach().numpy().flatten()
                all_params_list.append(params_np)
        
        if not all_params_list:
            print("Warning: No trainable parameters found in the model.")
            return

        all_params = np.concatenate(all_params_list)

        # 2. Calculate Statistics (for annotation)
        mean_val = all_params.mean()
        std_val = all_params.std()
        
        # 3. Set up the plot
        plt.figure(figsize=(12, 6))
        
        # 4. Plot Histogram
        # bins='auto' is usually fine, but fixed number (e.g., 100) gives more control
        n, bins, patches = plt.hist(all_params, bins=100, density=False,
                                    alpha=0.7, color='skyblue', edgecolor='black', label='Parameter Frequency')
        
        # 5. Apply Symmetrical Logarithmic Scale to X-axis
        # This allows showing negative, zero, and positive values on a log-like scale.
        # linthresh defines the range around 0 that remains linear (to handle log(0) error).
        # We set it dynamically based on the max absolute value to ensure the "tails" are visible.
        max_val = np.max(np.abs(all_params))
        lin_thresh = max_val * 0.01  # Keep 1% of the range linear around 0
        
        plt.xscale('symlog', linthresh=lin_thresh, base=10)
        
        # 6. Formatting
        plt.title(f'{model_name} - Parameter Distribution (SymLog X-Axis)', fontsize=16, fontweight='bold')
        plt.xlabel('Parameter Value (Symmetrical Log Scale)', fontsize=12)
        plt.ylabel('Frequency (Count)', fontsize=12)
        
        # Enable grid for better readability on log scales
        ...
        stats_text = (f'Mean: {mean_val:.4f}\n'
                    ...
                    f'Min: {all_params.min():.4f}\n'
                    f'Max: {all_params.max():.4f}\n'
                    f'Total Params: {len(all_params):,}')
        ...
        save_path = os.path.join(RESULTS_DIR, f"{model_name.replace(' ', '_')}_dist_xlog.png")
        plt.savefig(save_path, dpi=300)
        print(f"Plot saved to: {save_path}")
        plt.show()
    plot_mlp_param_distribution(net_u_mixed, model_name="Mixed Residual PINN")
